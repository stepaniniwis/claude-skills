# dms-Einreichungscheckliste

Paketinhalt:
- `01-titelseite.md` – Titelseite mit Platzhaltern (Autorenschaft, COI, Freigabe)
- `02-manuskript-anonym.docx` / `.md` – anonymisierte Begutachtungsfassung
- `03-begleitschreiben.md` – Anschreiben an die Redaktion
- `../manuskript-de.md` – Quellfassung (mit Abstracts DE/EN)
- `../data-appendix.md` – Datenanhang (auf Anfrage der Gutachter, nicht Teil der Ersteinreichung)

## Vor dem Upload zwingend (nur du kannst das)

- [ ] **Autorenschaft festlegen** und Titelseite ausfüllen (Namen, ORCID, Affiliation, korrespondierende E-Mail).
- [ ] **Freigabe der Landeshauptstadt München** (KVR/Wahlamt) für Nennung, Zahlen und Freitextzitate schriftlich einholen; Datum ins Manuskript (Abschnitt 4.5) und in die Titelseite.
- [ ] **Interessenkonflikt formulieren** (Verhältnis zur Stadt, ggf. zum Anbieter).
- [ ] **dms-Autorenhinweise im Browser abrufen** (budrich-journals.de → dms → "Über uns"/"Einreichungen"; für automatisierte Zugriffe gesperrt) und abgleichen:
  - [ ] max. Zeichenzahl Abhandlung (Manuskript aktuell ~49.000 Zeichen inkl. Leerzeichen und Literatur; bei Limit 40.000 → Kürzungsplan: Abschnitt 3.2 straffen, Tabelle 1 in Online-Anhang)
  - [ ] Abstractlänge (aktuell DE ~165 Wörter, EN ~185 Wörter)
  - [ ] Zitier-/Literaturformat (aktuell Budrich-üblich Autor-Jahr; ggf. an Stylesheet anpassen)
  - [ ] Datei-Format (OJS nimmt üblicherweise .docx)
- [ ] **Registrierung/Upload im OJS** auf budrich-journals.de (Rubrik "Abhandlungen"), Metadaten DE+EN eintragen.
- [x] Referenzen: alle `[prüfen]`-Marker aufgelöst; sämtliche Einträge über Verlags-/Datenbankquellen verifiziert (u. a. korrigiert: Budurushi/**Jöris**/Volkamer 2014 statt Neumann/Renaud; Kitcat/**Brown** statt Browning; Jerhov/Högström **2024**; Stein et al. **2025**; James/**Garnett/Asplund/Campion** 2023 statt "Clark"). **Restrisiko, am PDF gegenprüfen:** Heft/Seiten bei DeMora et al. 2022 (21(3), 249–258) und Hostetter/Atkeson 2025 (Online First) sind nur einfach belegt; Vorname "P. Jöst" (vermutlich Paula) am Frontiers-PDF prüfen.

## Inhaltlich erledigt (Stand heute)

- [x] Vollständiger Methodenteil inkl. Instrumentenbegründung (Tabelle 1, Ex-post-Konstruktzuordnung)
- [x] Theorierahmen (Davis 1989; Venkatesh et al. 2003; Rogers 2003; Lipsky 1980)
- [x] Statistische Korrektur des Signifikanz-Claims aus dem städtischen Bericht (Fisher p = 0,30 transparent gemacht)
- [x] Abstract DE + EN mit Keywords
- [x] Anonymisierte Begutachtungsfassung (ohne Autorenbezüge/Danksagung)
- [x] Alle Zahlen gegen Rohexporte reproduziert (`data-appendix.md`)
- [x] Unbelegte Kontextbehauptungen entfernt; BVerfGE 123, 39 und Art. 34 GLKrWG ergänzt
- [x] Unsichere Referenz (Glaser et al. 2015, unklarer Erscheinungsort) entfernt und ersetzt

## Optional, stärkt die Einreichung

- [ ] Abbildung 1: Prozessvergleich manuelle vs. digitale Auszählung (Schwimmbahnen-Diagramm)
- [ ] Online-Anhang mit `data-appendix.md` (Transparenz-Plus im Review)
- [ ] Gutachter*innen-Vorschläge (Wahlorganisation / Verwaltungsdigitalisierung, ohne Interessenkonflikt zur Stadt)
