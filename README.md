...
- **[argument-architecture](skills/build/argument-architecture/)** — CARS model (macro: Introduction architecture, research space creation) plus Toulmin (micro: claim-evidence-warrant for each argument) plus a Substitution Stress Test for new theoretical concepts: name the three nearest neighbours an editor would reach for, map their coverage, specify the residual only your concept explains. Three levels of structure for academic positioning.
